...
CONFIG_SCHEMA = cv.All(
    ...
    .extend(
        cv.Schema(
            {
                cv.Required(CONF_GPIO_SCLK ): cv.gpio_sclk,
                cv.Required(CONF_GPIO_MISO ): cv.gpio_miso,
                cv.Required(CONF_GPIO_MOSI ): cv.gpio_mosi,
                cv.Required(CONF_GPIO_CS   ): cv.gpio_cs,
                cv.Required(CONF_GPIO_GDO0 ): cv.gpio_gdo0,
                cv.Required(CONF_GPIO_GDO2 ): cv.gpio_gdo2,
                cv.Required(CONF_BITRATE   ): cv.bitrate,
                cv.Required(CONF_FREQUENCY ): cv.frequency,
                cv.Required(CONF_TX_POWER  ): cv.txPower,
                cv.Required(CONF_DATARATE  ): cv.dataRate,
                cv.Required(CONF_MODULATION): cv.modulation,
                cv.Required(CONF_REPEAT    ): cv.repeat,
            }
        ),
    ),
